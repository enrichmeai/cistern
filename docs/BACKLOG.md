...
- [x] **T7.14 First tagged release.** `release.yml` on `v*` tags: version from tag, full tests,
  ...
  once.*
- [x] **T7.16 Integration kit — brokered sign-in with Google Workspace via Keycloak, so a
  small company's staff sign in with what they have.** `integration-kit/keycloak/build-realm.py`
  gains identity-provider steps: Keycloak's built-in `google` provider from `GOOGLE_CLIENT_ID`,
  `GOOGLE_CLIENT_SECRET` and `GOOGLE_HOSTED_DOMAIN` in `identities.env` (blank by default, in
  which case the step is skipped and the kit behaves exactly as before); trust email,
  first-broker-login without the review-profile prompt, hosted domain restricted to the
  company's Workspace domain. A brokered person's WebID is Cistern's `webid-template` over
  Keycloak's stable `sub` — no per-user admin step; decision, shape and reason in the kit README.
  Proven real-first against a second realm in the same Keycloak standing in for the external
  IdP (`external-idp`, carol) and captured by `capture-fixtures.sh` as
  `fixtures/token-carol-via-broker.*`; the Google-brokered capture needs the owner's OAuth
  client and follows the README runbook. Microsoft 365 has the same shape and is not built. DoD:
  blank Google values → `docker compose up` and the fixtures behave exactly as before; carol,
  who exists only behind the broker, signs in through the `cistern` realm, receives
  `aud: cistern`, and gets 200 on a resource granted to her template-derived WebID and 403 on
  one that is not; `realm-cistern.json` regenerated by the script, never hand-edited. Owner
  direction 2026-09-10. Issue #199.
- [x] **T7.17 `cistern sync`.** Mirror a local folder into a pod container from `cistern-cli`:
  sub-folders become containers, files are `PUT` under a closed extension→media-type table (RDF
  types sent as such, never parsed by the CLI), every write conditional (`If-None-Match: *` for
  a file never sent, `If-Match: <etag>` for one sent before; 412 → exit 3, never retried blind),
  a state file `<local-dir>/.cistern-sync.json` (path → etag + sha256) so a re-run sends only
  what changed, `--dry-run`, `--delete` (off by default). Not built: watch mode, Drive or
  SharePoint APIs, parallel uploads, RDF handling, symlink following. Owner direction 2026-09-10
  — small companies' documents have to get into a pod without a hand-written `PUT` loop; issue
  #200, outside the original numbering as T7.5 is. DoD: `CliEndToEndTest` fixture tree
  round-trips byte-identical with the right `Content-Type`; second run zero PUTs; one change →
  one `If-Match` PUT; `--delete` removes; stale etag → CONFLICT; `--dry-run` writes nothing;
  `cistern --help` and INTEGRATION.md §6.6; follow-up filed to replace the demo and kit seed
  loops (#201).
