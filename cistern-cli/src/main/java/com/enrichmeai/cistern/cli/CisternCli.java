 ...
 * (T5.6), {@code sync} (T7.17), over HTTP against a running server, with the caller's own
 * credential.
 *
 ...
 * {@code cistern pod create} exactly the {@code PUT <root>/} and {@code PUT <root>/.acl}, a
 * {@code cistern sync} exactly the conditional {@code PUT}s of each file, and each is refused
 * exactly when that would be. What the tool adds is that the file it writes is always the
 * right shape (see {@code GrantService} and {@code PodProvisioner}): the owner is never locked
 * out, a container grant always reaches inside the container, a new pod's owner holds
 * everything and nobody else anything, a folder reaches the pod without a hand-written loop
 * and without ever overwriting a copy that changed there, and the result is read back and
...
@Command(name = Usage.COMMAND_NAME, description = Usage.COMMAND_DESCRIPTION,
        ...
        subcommands = {PodCommand.class, SyncCommand.class, GrantCommand.class, RevokeCommand.class},
...
public final class CisternCli {
    ...
    static ExitCode exitCodeFor(Throwable failure) {
        return switch (failure) {
            ...
            case CliFailure.ContainerNotEmpty _ -> ExitCode.FAILURE;
            case CliFailure.LocalFolder _ -> ExitCode.FAILURE;
